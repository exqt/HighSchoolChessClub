...
#include "CCSettingsLocal.h"
#include "SoundControlBus.h"
#include "SoundControlBusMix.h"
#include "AudioModulationStatics.h"
#include "Audio/CCAudioSettings.h"

UCCSettingsLocal* UCCSettingsLocal::Get()
{
	return GEngine ? CastChecked<UCCSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UCCSettingsLocal::SetOverallVolume(float InVolume)
{
...
void UCCSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UCCSettingsLocal::SetSFXVolume(float InVolume)
{
	// Cache the incoming volume value
	SFXVolume = InVolume;
...
void UCCSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UCCAudioSettings* AudioSetting = GetDefault<UCCAudioSettings>())
			{
				...
				if (UObject* ObjPath = AudioSetting->OverallVolumeControlBus.TryLoad())
				{
				...
				if (UObject* ObjPath = AudioSetting->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = AudioSetting->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
				...
				if (UObject* ObjPath = AudioSetting->UserSettingsControlBusMix.TryLoad())
				{
...
void UCCSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
